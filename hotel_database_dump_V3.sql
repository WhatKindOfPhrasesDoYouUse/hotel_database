...
1	Уборка номера	Ежедневная базовая уборка	200	5	2
2	Замена полотенец	Свежие полотенца по требованию	100	5	2
3	Wi-Fi	Бесплатный интернет в номере	150	5	4
4	Доставка воды	Бутылка воды по запросу	50	5	4
5	Будильник	Услуга телефонного пробуждения	30	5	1
6	Телевидение	Цифровое ТВ с основными каналами	180	5	3
7	Мини-сейф	Использование сейфа в номере	80	5	1
8	Уборка номера	Глубокая уборка раз в три дня	200	6	2
9	Замена постельного белья	Раз в два дня	120	6	2
10	Доп. подушка	По запросу гостя	40	6	4
11	Чай и кофе	Комплект чая и кофе ежедневно	70	6	4
12	Wi-Fi	Скоростной интернет	150	6	4
15	Ежедневная уборка	Премиальная уборка каждый день	0	7	2
16	Замена полотенец и халатов	Каждый день	150	7	2
17	Room service	Еда и напитки в номер	250	7	4
18	Спа-набор	Набор косметики и ухода	150	7	2
20	Персональный консьерж	Помощь в бронировании/покупках	300	7	1
22	Техобслуживание по требованию	Любые неполадки – быстрое реагирование	220	7	3
25	Wi-Fi	Бесплатный высокоскоростной интернет	150	8	4
27	Room service	24/7 обслуживание	250	8	4
28	Сменный халат	Чистый халат ежедневно	90	8	2
33	Замена полотенец	По требованию	100	9	2
34	Wi-Fi	Бесплатно	150	9	4
35	Чайник и чай	Комплект в номере	60	9	4
37	Будильник	По звонку	30	9	1
19	Wi-Fi+	Усиленный интернет	220	7	4
23	Ежедневная уборка	Полная уборка и дезинфекция	0	8	2
31	Техобслуживание	В любое время	200	8	3
32	Уборка	Стандартная уборка	200	9	2
36	Телевизор	Стандартные каналы	150	9	3
\.
...
COPY core.amenity_booking (id, order_date, order_time, ready_date, ready_time, completion_status, quantity, amenity_id, guest_id, employee_id, room_booking_id) FROM stdin;
\.
...
COPY core.amenity_review (id, comment, publication_date, publication_time, rating, guest_id, amenity_id, amenity_booking_id) FROM stdin;
\.
...
86	3234567812345678	03/11	19	13
\.
...
39	Ирина	Зукова	Николаевна	ir.zuk@gmail.com	+79222314141	AQAAAAIAAYagAAAAEEN7n+I2jSChYDDX3S/bjKwbN7kWPIp641TQNHHVzX4PJd+pAG8sj+yl3VumFPDVmA==
20	Дмитрий	Ошуев	Владимирович	dima@gmail.com	+79229314150	AQAAAAIAAYagAAAAEDD+Yd4XqbvPjk3eXnjP/gQPSoaCZjYiaYY4oesd/a0v7+Qu2XdZ4jt6SSe5w2JKlg==
44	Матвей	Кожин	Сергеевич	matvik@gmail.com	+79229313151	AQAAAAIAAYagAAAAENEKx3M/6p3VZmQH+3LXh4730VnlfmJnyNyRhbooSoHg8vXf79pvs4RXT5Uw8qhQjA==
45	Александра	Халтурина	Владимирович	sasha@yandex.ru	+79229513110	AQAAAAIAAYagAAAAEK+XMpqBURFMfxxkTiC4ELcEL5CKakUMjWUq/o/XRKxjaI8AuF/hBeB8etDtauAiUg==
\.
...
COPY core.hotel_review (id, comment, publication_date, publication_time, rating, guest_id, hotel_id, room_booking_id) FROM stdin;
\.
...
8	4	Description room 4	3	6000	1
\.
...
SELECT pg_catalog.setval('core.additional_guest_id_seq', 13, true);
...
SELECT pg_catalog.setval('core.amenity_booking_id_seq', 25, true);
...
SELECT pg_catalog.setval('core.amenity_payment_id_seq', 14, true);
...
SELECT pg_catalog.setval('core.amenity_review_id_seq', 6, true);
...
SELECT pg_catalog.setval('core.card_id_seq', 88, true);
...
SELECT pg_catalog.setval('core.client_id_seq', 45, true);
...
SELECT pg_catalog.setval('core.employee_id_seq', 23, true);
...
SELECT pg_catalog.setval('core.guest_id_seq', 20, true);
...
SELECT pg_catalog.setval('core.hotel_review_id_seq', 27, true);
...
SELECT pg_catalog.setval('core.room_booking_id_seq', 95, true);
...
SELECT pg_catalog.setval('core.room_payment_id_seq', 28, true);
...
--
-- Name: ux_amenity_review_amenity_booking_id; Type: INDEX; Schema: core; Owner: postgres
--

CREATE UNIQUE INDEX ux_amenity_review_amenity_booking_id ON core.amenity_review USING btree (amenity_booking_id);


--
-- Name: ux_hotel_review_booking; Type: INDEX; Schema: core; Owner: postgres
--

CREATE UNIQUE INDEX ux_hotel_review_booking ON core.hotel_review USING btree (room_booking_id);
...
--
-- Name: amenity_booking amenity_booking_room_booking_id_fkey; Type: FK CONSTRAINT; Schema: core; Owner: postgres
--

ALTER TABLE ONLY core.amenity_booking
    ADD CONSTRAINT amenity_booking_room_booking_id_fkey FOREIGN KEY (room_booking_id) REFERENCES core.room_booking(id) ON UPDATE CASCADE ON DELETE CASCADE;
...
--
-- Name: amenity_review fk_amenity_review_amenity_booking; Type: FK CONSTRAINT; Schema: core; Owner: postgres
--

ALTER TABLE ONLY core.amenity_review
    ADD CONSTRAINT fk_amenity_review_amenity_booking FOREIGN KEY (amenity_booking_id) REFERENCES core.amenity_booking(id) ON DELETE CASCADE;


--
-- Name: hotel_review fk_hotel_review_booking; Type: FK CONSTRAINT; Schema: core; Owner: postgres
--

ALTER TABLE ONLY core.hotel_review
    ADD CONSTRAINT fk_hotel_review_booking FOREIGN KEY (room_booking_id) REFERENCES core.room_booking(id) ON UPDATE CASCADE ON DELETE CASCADE;
